        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
          <div>
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 max-w-xl">
              Scalable single sign-on gateways and real-time telemetry dashboards architected with strict Feature-Sliced Design (FSD) and modern React tooling.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {filteredProjects.map((project) => {
            ...
            return (
              <div
                key={project.id}
                className="glass-card glass-card-hover rounded-3xl p-6 sm:p-8 flex flex-col justify-between relative overflow-hidden group shadow-xl"
              >
                {/* Top Accent Light */}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h3 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white group-hover:text-sky-500 dark:group-hover:text-sky-400 transition-colors">
                          {project.title}
                        </h3>
                    ...
                    <span className="px-2.5 py-1 rounded-full text-[11px] font-medium bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 flex items-center gap-1">
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-6">
                    {project.metrics.map((m, mIdx) => (
                      ...
                      >
                        <div className="text-sm font-mono font-bold text-sky-600 dark:text-sky-400">{m.value}</div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-6">
                    {project.techStack.slice(0, 7).map((tech, tIdx) => (
                      <span 
                        key={tIdx} 
                ...
                {/* Card Action Buttons */}
                <div className="flex flex-wrap items-center gap-2 pt-4 border-t border-slate-200/80 dark:border-slate-800/80">
                  <button
                    onClick={() => onOpenProjectModal(project)}
                    className="flex-1 py-2.5 px-4 rounded-xl text-xs font-semibold text-white bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-400 hover:to-indigo-500 shadow-md shadow-sky-500/20 hover:shadow-sky-500/30 transition-all flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95 cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    title="Inspect FSD Architecture Snippets"
                  >
