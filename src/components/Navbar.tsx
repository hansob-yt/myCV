  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1 bg-slate-100/80 dark:bg-slate-900/70 p-1.5 rounded-full border border-slate-200/80 dark:border-slate-800/80 backdrop-blur-md">
          {navLinks.map((link) => {
        ...
        {/* Actions (Code Explorer, PDF Resume, Day/Night Toggle, Atmosphere Switcher) */}
        <div className="hidden sm:flex items-center gap-2">
          {/* Terminal / Code Explorer Button */}
          <button
            onClick={onOpenTerminal}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800/80 hover:bg-sky-500/10 hover:text-sky-600 dark:hover:text-sky-400 border border-slate-200 dark:border-slate-700/60 transition-all hover:scale-105 active:scale-95 cursor-pointer"
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-400 hover:to-indigo-500 shadow-md shadow-sky-500/20 hover:shadow-sky-500/30 transition-all hover:scale-105 active:scale-95 cursor-pointer"
          ...
          <button
            onClick={toggleMode}
            className="p-2 rounded-lg text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700/60 transition-all hover:scale-105 active:scale-95 cursor-pointer"
          ...
          {/* Atmosphere Background Palette Switcher */}
          <button
            onClick={cycleAtmosphere}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-mono font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700/60 transition-all hover:scale-105 active:scale-95 cursor-pointer group"
            title={`Atmosphere Mood: ${themeConfig.name} (Click to change)`}
          >
            <Palette className="w-3.5 h-3.5 text-sky-500 group-hover:rotate-45 transition-transform" />
            <span className="hidden lg:inline text-[11px]">{themeConfig.name}</span>
          </button>
        </div>
        ...
        <div className="flex sm:hidden items-center gap-1.5">
          <button
            onClick={toggleMode}
            className="p-2 rounded-lg text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 cursor-pointer"
            aria-label="Toggle day/night mode"
          >
