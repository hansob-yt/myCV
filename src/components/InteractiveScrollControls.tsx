  ...
  return (
    <>
      ...
      >
        ...
        <div className="pt-2 border-t border-slate-200/60 dark:border-slate-800/60 text-[10px] font-mono text-slate-400">
          {Math.round(scrollProgress)}%
        </div>
      </aside>

      {/* 3. Floating Quick Bottom Action Button */}
      {isVisible && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-6 right-6 z-40 p-3 rounded-2xl bg-gradient-to-tr from-sky-500 to-indigo-600 text-white shadow-lg shadow-sky-500/30 hover:scale-110 active:scale-95 transition-all duration-300 flex items-center justify-center no-print group cursor-pointer"
          title="Scroll to Top"
