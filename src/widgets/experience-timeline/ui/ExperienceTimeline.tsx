      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-12 gap-4">
          ...
          <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 font-mono">
            2 Years at Behsam Kavan Araz (Kavano)
          </p>
        </div>

        {/* Minimalist Timeline Cards Grid */}
        <div className="relative pl-6 sm:pl-10 space-y-8">
          {/* Vertical Connecting Line */}
          <div className="absolute left-3 sm:left-5 top-4 bottom-4 w-0.5 bg-linear-to-b from-sky-500 via-indigo-500 to-slate-300 dark:to-slate-800" />
          ...
          {experiences.map((exp) => {
            ...
            return (
              ...
              >
                {/* Node Icon */}
                <div
                  className={`absolute -left-6.75 sm:-left-8.75 top-4 w-10 h-10 rounded-2xl flex items-center justify-center border transition-all duration-300 shadow-md ${
                    isWork
                      ? "bg-linear-to-tr from-sky-500 to-indigo-600 border-sky-400 text-white shadow-sky-500/30 group-hover:scale-110"
                ...
                {/* Minimalist Card */}
                <div className="glass-card glass-card-hover rounded-2xl p-5 sm:p-7 border border-slate-200/90 dark:border-slate-800/80 shadow-md flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-slate-800 dark:text-slate-300 leading-relaxed mb-4">
                      {exp.summary}
                    </p>

                    {/* Top Tech Stack Pills */}
                    <div className="flex flex-wrap gap-1.5 mb-5">
                      {exp.techStack.slice(0, 5).map((tech, tIdx) => (
                        <span
                          key={tIdx}
                          className="px-2 py-0.5 rounded-md text-[11px] font-mono bg-slate-100 dark:bg-slate-800/90 text-slate-800 dark:text-slate-300 border border-slate-200 dark:border-slate-700/60 font-medium"
                        >
                          {tech}
                        </span>
                      ))}
                      {exp.techStack.length > 5 && (
                        <span className="px-2 py-0.5 rounded-md text-[11px] font-mono text-slate-500 dark:text-slate-400">
                  ...
                  <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
                    <span className="text-xs font-mono text-slate-500 dark:text-slate-400">
                      {exp.accomplishments.length} key achievements •{" "}
                      {exp.metrics.length} metrics
                    </span>
